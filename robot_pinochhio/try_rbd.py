...
class PinocchioTest(Node):
    ...
    def listener_callback(self, msg):
        ...
        endeff_j = pin.computeFrameJacobian(model, self.data, q, ENDEF_FRAME_ID, pin.WORLD)
        # print(np.round(full_body_j,decimals=2))
        print("Frame of endeffector \n" ,self.data.oMf[ENDEF_FRAME_ID])
        print("Jacobian of endeffector \n", np.round(endeff_j,decimals = 2))
        print("Jocabian of arm tool frame \n", np.round(pin.computeFrameJacobian(model, self.data, q, 73, pin.WORLD), decimals=2))


    def model_from_msg(self, msg):
        return pin.buildModelFromXML(msg.data)
    
    def log_joint_names(self):
        for i in range(len(self.model.names)):
            self.get_logger().info(f"Joint with joint id {i} is {self.model.names[i]}")

    def log_transforms(self):
        for oMi, name in zip(self.data.oMi,self.model.names):
            self.get_logger().info(f"Transform for joint {name} is: \n {oMi.homogeneous}")

    def log_frame_names(self):
        for i in range(len(self.model.frames)):
            self.get_logger().info(f"Frame with frame id {i} is {self.model.frames[i].name}")
